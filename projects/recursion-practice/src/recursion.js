...
var sum = function(array) {
  ...
  // recursive case
  // return array at index 0 plus sliced array by 1
...
// return subtracting x plus function call with input parameters x, (y + 1)
    return - x + multiply(x, y + 1);
}
// otherwise
else {
// return x plus function call with input parameters x, (y + 1)
    return x + multiply(x, y - 1);
}
};
...
var reverseArr = function (arr, acc = []) {
  if (arr.length) { // if there are elements in the array, which there aren't!
    const [ head, ...tail ] = arr;
    return reverseArr(tail, [head].concat(acc));
  } else {
    // we're done here, return the accumulator.
    return acc;
  }
}
